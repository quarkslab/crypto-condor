   ...
   )
{
   ...
   printf (":\n\n       Test                                             p-value\n");
   printf (" -----------------------------------------------------------------\n");
   printf ("     (PASS if %f <= p-value <= %f)\n", gofw_Suspectp, 1.0 - gofw_Suspectp);

   for (j = 0; j < N; j++) {
      if (pVal[j] < 0.0)          /* That test was not done: pVal = -1 */
         continue;
      printf (" %2d ", TestNumber[j]);
      printf (" %-50s", bbattery_TestNames[j]);
      if ((pVal[j] >= gofw_Suspectp) && (pVal[j] <= 1.0 - gofw_Suspectp))
         printf (" PASS %f", pVal[j]);
      else if (isnan(pVal[j]))
         printf (" NOT IMPLEMENTED");
      else
         printf (" FAIL %f", pVal[j]);
      WritepVal (pVal[j]);
      printf ("\n");
   }
   printf (" -----------------------------------------------------------------\n");
...
static void GetPVal_Walk_NIST (long N, swalk_Res * res, int *pj, char *mess)
/*
 * Get the p-values in a swalk_RandomWalk1 test
 */
{
   ...
   if (N == 1) {
      ...
      bbattery_pVal[++j] = res->M[0]->pVal2[gofw_Mean];
      TestNumber[j] = 13;
      strcpy (CharTemp, "Cumulative Sum (Cusum) Test");
      strncat (CharTemp, mess, (size_t) len);
      strncpy (bbattery_TestNames[j], CharTemp, (size_t) LEN);
      ...
      bbattery_pVal[++j] = res->R[0]->pVal2[gofw_Mean];
      TestNumber[j] = 15;
      strcpy (CharTemp, "Random Excursions Variant Test");
      strncat (CharTemp, mess, (size_t) len);
      strncpy (bbattery_TestNames[j], CharTemp, (size_t) LEN);
 ...
 */
{
   ...
   }
   {
      ...
      for (i = 0; i < Rep[j2]; ++i) {
         ...
         TestNumber[j] = j2;
         strcpy (bbattery_TestNames[j], "LinearComp, r = 0 [1/2]");
         bbattery_pVal[++j] = res->JumpSize->pVal2[gofw_Mean];
         TestNumber[j] = j2;
         strcpy (bbattery_TestNames[j], "LinearComp, r = 0 [2/2]");
      }
      ...
      for (i = 0; i < Rep[j2]; ++i) {
         ...
         bbattery_pVal[++j] = res->JumpNum->pVal2[gofw_Mean];
         TestNumber[j] = j2;
         strcpy (bbattery_TestNames[j], "LinearComp, r = 29 [1/2]");
         bbattery_pVal[++j] = res->JumpSize->pVal2[gofw_Mean];
         TestNumber[j] = j2;
         strcpy (bbattery_TestNames[j], "LinearComp, r = 29 [2/2]");
      }
      scomp_DeleteRes (res);
   }
   {
 ...
 */
{
   ...
   for (i = 0; i < Rep[j2]; ++i) {
      ...
      GetPVal_Walk (N, res, pj, stringL, j2);
   }
   if (L < 96)
      return;
...
// Taken from DoWalk, but adapted to our needs in NIST
static void DoWalk_NIST(
   unif01_Gen * gen,              /* */
   double nb,                     /* Number of bits to test */
   int *pj,                       /* j */
   int *j2                        /* Test number in the battery */
 ...
 */
{
  ...
  ufile_InitReadBin();
  swalk_RandomWalk1(gen, res, N, n, r, s, L, L);
  GetPVal_Walk_NIST(N, res, pj, stringL);
  if (L < 96)
    return;
  ...
  ufile_InitReadBin();
  swalk_RandomWalk1(gen, res, N, n, r, s, L, L);
  GetPVal_Walk_NIST(N, res, pj, " (L = 1024)");
  ...
  ufile_InitReadBin();
  swalk_RandomWalk1(gen, res, N, n, r, s, L, L);
  GetPVal_Walk_NIST(N, res, pj, " (L = 10016)");
 ...
 */
{
...

   {
      ...
      for (i = 0; i < Rep[j2]; ++i) {
         ...
         TestNumber[j] = j2;
         strcpy (bbattery_TestNames[j], "LinearComp [1/2]");
         j++;
         if (N == 1)
            bbattery_pVal[j] = res->JumpNum->pVal2[gofw_Mean];
         else
         ...
         TestNumber[j] = j2;
         strcpy (bbattery_TestNames[j], "LinearComp [2/2]");
      }
      scomp_DeleteRes (res);
   }
...
// Most of this function comes from the Rabbit or Alphabit functions
void NIST(char* fname, double nb)
{
  // keeping the s most significant bits of each (after discarding the first r),
  const int s = 32;
  const int r = 0;
  ...
  // TODO: we need to define how many bits we want to be read
  util_Assert(nb >= 500.0, "bbattery_NIST:   nb < 500");
   ...
   unif01_Gen* gen = ufile_CreateReadBin(fname, bufsiz);

   // Taken from Crush and BigCrush
  {
    sres_Basic* res;
    res = sres_CreateBasic();
    ++j2;
    sstring_HammingWeight2(gen, res, 1, nb, r, s, nb);
    bbattery_pVal[++j] = res->pVal2[gofw_Sum];
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Frequency (Monobit) Test");
    sres_DeleteBasic(res);
    ...
    res = sres_CreateBasic();
    ++j2;
    sstring_HammingWeight2(gen, res, 1, nb, r, s, L);
    bbattery_pVal[++j] = res->pVal2[gofw_Sum];
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Test For Frequency Within A Block");
    sres_DeleteBasic(res);
  }
  ...
  // Taken from Rabbit
  {
    ...
    if (N == 1)
      ...
      bbattery_pVal[j] = res->NRuns->pVal2[gofw_Sum];
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Runs Test");
    // TODO: why there is a duplicate there in Rabbit?
    /*j++;*/
    /*if (N == 1)*/
      /*bbattery_pVal[j] = res->NBits->pVal2[gofw_Mean];*/
    /*else*/
  ...
  // Taken from Rabbit
  {
    ...
    if ((L >= 1032) && (n >= 30)) {
      ufile_InitReadBin ();
      sstring_LongestHeadRun(gen, res, 1, n, r, s, L);
      j++;
      bbattery_pVal[j] = res->Chi->pVal2[gofw_Mean];
      TestNumber[j] = j2;
      strcpy(bbattery_TestNames[j], "Test For The Longest Run of Ones in a Block");
    }
    sstring_DeleteRes2(res);
  }
  ...
  // Taken from Rabbit
  {
    ...
    if (n >= 50) {
      ufile_InitReadBin();
      j++;
      smarsa_MatrixRank(gen, res, 1, n, r, s, s, s);
      bbattery_pVal[j] = res->pVal2[gofw_Mean];
      TestNumber[j] = j2;
      strcpy(bbattery_TestNames[j], "Random Binary Matrix Rank Test, 32 x 32");
    }
    ...
    if (n >= 50) {
      ufile_InitReadBin();
      j++;
      smarsa_MatrixRank(gen, res, 1, n, r, s, 10 * s, 10 * s);
      bbattery_pVal[j] = res->pVal2[gofw_Mean];
      TestNumber[j] = j2;
      strcpy(bbattery_TestNames[j], "Random Binary Matrix Rank Test, 320 x 320");
    }
    ...
    if (n >= 50) {
      ufile_InitReadBin();
      j++;
      smarsa_MatrixRank(gen, res, 1, n, r, s, 32 * s, 32 * s);
      bbattery_pVal[j] = res->pVal2[gofw_Mean];
      TestNumber[j] = j2;
      strcpy(bbattery_TestNames[j], "Random Binary Matrix Rank Test, 1024 x 1024");
    }
    sres_DeleteChi2(res);
  }
  ...
  // Taken from Rabbit
  {
    ...
    ufile_InitReadBin();
    ++j2;
    sspectral_Fourier1(gen, res, 1, k, r, s);
    j++;
    bbattery_pVal[j] = res->Bas->pVal2[gofw_Mean];
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Discrete Fourier Transform (Spectral) Test");
    sspectral_DeleteRes(res);
  }
  ...
  // TODO
  {
    ...
    ++j;
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Non-Overlapping (Aperiodic) Template Matching Test");
    bbattery_pVal[j] = -NAN;
  }
  ...
  /*{*/
    ...
    ++j;
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Overlapping (Periodic) Template Matching Test");
    bbattery_pVal[j] = -NAN;
    /*// TODO: are we sure we want here to use Delta = 1*/
    /*int NbDelta = 1;*/
    /*double ValDelta[] = { 1 };*/
    /*smultin_Param *par = NULL;*/
    /*smultin_Res *res;*/
    ...
    /*++j2;*/
    /*smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 2, FALSE);*/
    /*strcpy(bbattery_TestNames[++j], "Non-overlapping Template Matching Test, L = 2");*/
    /*if (N == 1)*/
      /*bbattery_pVal[j] = res->pVal2[0][gofw_Mean];*/
    /*else*/
      /*bbattery_pVal[j] = res->pVal2[0][gofw_AD];*/
    ...
    /*++j2;*/
    /*smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 4, FALSE);*/
    /*strcpy(bbattery_TestNames[++j], "Non-overlapping Template Matching Test, L = 4");*/
    /*if (N == 1)*/
      /*bbattery_pVal[j] = res->pVal2[0][gofw_Mean];*/
    /*else*/
      /*bbattery_pVal[j] = res->pVal2[0][gofw_AD];*/
    ...
    /*if (n > 250) {*/
      /*ufile_InitReadBin();*/
      /*smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 8, FALSE);*/
      /*strcpy(bbattery_TestNames[++j], "Non-overlapping Template Matching Test, L = 8");*/
      /*if (N == 1)*/
        /*bbattery_pVal[j] = res->pVal2[0][gofw_Mean];*/
      /*else*/
        /*bbattery_pVal[j] = res->pVal2[0][gofw_AD];*/
    ...
    /*if (n > 65000) {*/
      /*ufile_InitReadBin();*/
      /*smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 16, FALSE);*/
      /*strcpy(bbattery_TestNames[++j], "Non-overlapping Template Matching Test, L = 16");*/
      /*if (N == 1)*/
        /*bbattery_pVal[j] = res->pVal2[0][gofw_Mean];*/
      /*else*/
        /*bbattery_pVal[j] = res->pVal2[0][gofw_AD];*/
  ...
  // Taken from DoAppear
  {
    ...
    if (N == 1)
      ...
      bbattery_pVal[j] = res->pVal2[gofw_Sum];
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Maurer's Universal Statistical Test");
    sres_DeleteBasic(res);
  }
  ...
  // Taken from Rabbit
  {
    ...
    if (N == 1)
      ...
      bbattery_pVal[j] = res->JumpSize->pVal2[gofw_Sum];
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Linear Complexity Test");
    //strcpy(bbattery_TestNames[j], "Linear Complexity Test [1/2]");
    j++;
    // TODO: not used
    /*if (N == 1)*/
      /*bbattery_pVal[j] = res->JumpNum->pVal2[gofw_Mean];*/
    /*else*/
      /*bbattery_pVal[j] = res->JumpNum->pVal2[gofw_Sum];*/
    /*TestNumber[j] = j2;*/
    /*strcpy(bbattery_TestNames[j], "Linear Complexity Test [2/2]");*/
    scomp_DeleteRes(res);
  }
  ...
  // Taken from Alphabit
  {
    ...
    ++j2;
    smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 2, FALSE);
    strcpy(bbattery_TestNames[++j], "Serial Test, L = 2");
    if (N == 1)
      bbattery_pVal[j] = res->pVal2[0][gofw_Mean];
    else
      bbattery_pVal[j] = res->pVal2[0][gofw_AD];
    ...
    ufile_InitReadBin();
    smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 4, FALSE);
    strcpy(bbattery_TestNames[++j], "Serial Test, L = 4");
    if (N == 1)
      bbattery_pVal[j] = res->pVal2[0][gofw_Mean];
    else
      bbattery_pVal[j] = res->pVal2[0][gofw_AD];
    ...
    if (n > 250) {
      ufile_InitReadBin();
      smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 8, FALSE);
      strcpy(bbattery_TestNames[++j], "Serial Test, L = 8");
      if (N == 1)
        bbattery_pVal[j] = res->pVal2[0][gofw_Mean];
      else
        bbattery_pVal[j] = res->pVal2[0][gofw_AD];
    ...
    if (n > 65000) {
      ufile_InitReadBin();
      smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 16, FALSE);
      strcpy(bbattery_TestNames[++j], "Serial Test, L = 16");
      if (N == 1)
        bbattery_pVal[j] = res->pVal2[0][gofw_Mean];
      else
        bbattery_pVal[j] = res->pVal2[0][gofw_AD];
...

  {
    ...
    ++j;
    TestNumber[j] = j2;
    strcpy(bbattery_TestNames[j], "Approximate Entropy Test");
    bbattery_pVal[j] = -NAN;
  // Taken from Alphabit
  /**
    ...
    ++j2;
    smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 2, FALSE);
    strcpy(bbattery_TestNames[++j], "Approximate Entropy Test, L = 2");
    if (N == 1)
      bbattery_pVal[j] = res->pVal2[0][gofw_Mean];
    else
      bbattery_pVal[j] = res->pVal2[0][gofw_AD];
    ...
    ++j2;
    smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 4, FALSE);
    strcpy(bbattery_TestNames[++j], "Approximate Entropy Test, L = 4");
    if (N == 1)
      bbattery_pVal[j] = res->pVal2[0][gofw_Mean];
    else
      bbattery_pVal[j] = res->pVal2[0][gofw_AD];
    ...
    if (n > 250) {
      ufile_InitReadBin();
      smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 8, FALSE);
      strcpy(bbattery_TestNames[++j], "Approximate Entropy Test, L = 8");
      if (N == 1)
        bbattery_pVal[j] = res->pVal2[0][gofw_Mean];
      else
        bbattery_pVal[j] = res->pVal2[0][gofw_AD];
    ...
    if (n > 65000) {
      ufile_InitReadBin();
      smultin_MultinomialBitsOver(gen, par, res, N, n, r, s, 16, FALSE);
      strcpy(bbattery_TestNames[++j], "Approximate Entropy Test, L = 16");
      if (N == 1)
        bbattery_pVal[j] = res->pVal2[0][gofw_Mean];
      else
        bbattery_pVal[j] = res->pVal2[0][gofw_AD];
  ...
  */
  ...
  // TODO: sentrop_EntropyDiscOver or sentrop_EntropyDiscOver2

  DoWalk_NIST(gen, nb, &j, &j2);
   
  util_Assert(j2 <= RABBIT_NUM, "NIST:   j2 > NIST_NUM");

  bbattery_NTests = ++j;
  WriteReport(fname, "NIST", bbattery_NTests,
      bbattery_pVal, Timer, TRUE, TRUE, nb);
  ufile_DeleteReadBin(gen);
  chrono_Delete(Timer);
}
